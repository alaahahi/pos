<template>
  <li class="flex gap-3 border-b shop-divider py-3 last:border-0">
    <img
      v-if="itemImageSrc(item)"
      :src="itemImageSrc(item)"
      :alt="item.name"
      class="h-14 w-14 shrink-0 rounded-lg object-cover bg-slate-100 dark:bg-slate-800"
      @error="(e) => onItemImageError(e, item)"
    />
    <div
      v-else
      class="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500"
      aria-hidden="true"
    >
      <i class="bi bi-image text-xl" />
    </div>
    <div class="min-w-0 flex-1">
      <p class="truncate text-sm font-medium shop-text-primary">{{ item.name }}</p>
      <p v-if="item.with_addon && item.addon_name" class="text-xs text-shop-600">
        + {{ item.addon_name }}
      </p>
      <p class="text-xs shop-text-muted">
        {{ formatPrice(item.price) }}
        <span class="text-[10px] font-medium text-slate-400 dark:text-slate-500">{{ item.currency || 'USD' }}</span>
        × {{ item.quantity }}
      </p>
